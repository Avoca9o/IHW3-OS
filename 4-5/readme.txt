...
Также он генерирует список координат минометов для каждой страны, но уже не записывает в 
файл, т.к. эти данные могут дублироваться при создании нового процесса при помощи команды fork()
именно для того, чтобы получать обновляющиеся данные, их пришлось записывать в файл.
Для того чтобы устранить возможные конфликты при обращении к файлам в программе
...
спрашиват у сервера соотношение сил, чтобы понять, не истреблена ли еще какая-либо команда
...
и неименованные семафоры, сигналы, а также потоки и дочерние процессы.
...
Результаты работы программы можно посмотреть на прикрепленной картинке в этой же папке.